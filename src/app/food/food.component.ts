...
@Component({
  ...
  <section class="mb-12">
      <form class="flex justify-center sm:justify-start">
        <input class="border-2 border-black-400 h-[40px] " type="text" placeholder="Filter by city" />
        <button class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Search</button>
      </form>
    </section>
    <section class="results h-screen flex flex-row flex-wrap sm:justify-start justify-center">
      <app-menu-card class="h-fit block" 
      *ngFor="let a of menuDataList"
        [menuData]="a">
    </app-menu-card>
  </section>
    `,
